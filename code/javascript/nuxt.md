...
I like to use Bulma, but there are some configurations I like to do to make the variables available to the components. (see below)
...
Move the code to the right place, including '.' files. Remove `node_modules` to keep the host clean. (they will be installed to the docker volume)
...
I prefer to keep static files outside of the ui/nuxt structure. The web / nginx layer is optimized for serving these files. 
This way the browser can cache the small static files. 

Keeping the files as part of the ui/static directory in the nuxt scaffolding may allow them to be bundled in with the application and minimize the number of requests necessary. 

TODO: (consider pros & cons to each approach) 


    rm -r static

TODO: 
any other template directories generated by nuxt scaffolding that can be removed?
